...
from get_feedback_data_copy import GetFeedbackDataCopy

feedback_data_obj = GetFeedbackDataCopy()

class PrepareModelDatasetCopy():
        def __init__(self):
        ...
        def generate_question(self, row):
                questions = [
                        f"識別子が「{row['project']}」、モジュールが「{row['module']}」、エージェントが「{row['agent']}」、障害状態が「{row['state']}」の場合、対応手順はどうなるでしょうか？",
                        f"識別子「{row['project']}」、モジュール「{row['module']}」、エージェント「{row['agent']}」、障害状態「{row['state']}」の場合に行う対応手順は何ですか?",
                        f"識別子「{row['project']}」、モジュール「{row['module']}」、エージェント「{row['agent']}」、および障害状態「{row['state']}」に基づいて発生されたシナリオの場合、標準的な対応手順は何になりますか?",
                        f"識別子が「{row['project']}」、モジュールが「{row['module']}」、エージェントが「{row['agent']}」、障害状態が「{row['state']}」の場合、推奨される対応手順は何ですか?"
                ]
                return questions
        ...
        def get_feedback_collected_data(self):
                df = feedback_data_obj.get_master_df()
        ...
        def preprocess_data(self):
                feedback_data_distinct_df = self.get_feedback_collected_data()
                ...
                feedback_data_distinct_df.loc[:, 'title_extraction_from_header'] = feedback_data_distinct_df.apply(lambda row: self.get_wiki_title(row['modified_generated_sop']), axis=1)
